...
const PAGES = {
  panels: {
    ...
    packages: [
      ['Акцентная стена', 'ТВ-зона, изголовье, прихожая или кабинет: подбираем ритм, цвет, фактуру и монтажный узел.'],
      ['Стена + двери', 'Собираем двери, панели, доборы и плинтус в одну линию, чтобы интерьер выглядел цельно.'],
      ['Скрытый проход', 'Помогаем спрятать техническую дверь или проход в стеновой отделке без визуального шума.'],
    ],
    specs: [
  ...
  partitions: {
    ...
    packages: [
      ['Кухня-гостиная', 'Легкая граница между зонами, которая сохраняет свет и ощущение открытого пространства.'],
      ['Кабинет или гардеробная', 'Перегородка под приватность, акуратный профиль, стекло и удобное открывание.'],
      ['Объект по проекту', 'Считаем секции, профиль, стекло, монтаж и сроки для квартиры, дома или коммерческого интерьера.'],
    ],
    specs: [
    ...
    <section class="solution-page">
      ...
      <section class="solution-packages reveal" aria-label="Пакеты решения">
        ...
        <div class="solution-packages__grid">
          ${page.packages.map(([title, text], i) => `
            <article>
              <span>${String(i + 1).padStart(2, '0')}</span>
              <strong>${title}</strong>
              <p>${text}</p>
